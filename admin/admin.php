<title>Admin</title>
    <div class="p-5"></div>
      ...
      <div class="table-responsive m-3">
        <span class="fs-4 ps-2 text-primary"><strong>Admin</strong></span>
<hr>
<div class="container col-5 ps-5">
<a class="btn btn-primary btn-sm container mb-4" href="index.php?halaman=tambahadmin"><b>Tambah Admin</b></a></div>
    ...
    $data = $koneksi->query("SELECT * FROM user WHERE level = 1");
    $jumlah_data = mysqli_num_rows($data);
    $jumlah_halaman = ceil($jumlah_data/$batas);

    $no=$halaman_awal+1;
    $ambil = $koneksi->query("SELECT * FROM user WHERE level = 1 ORDER BY id_akun LIMIT $halaman_awal,$batas");
...
<h6 class="text-primary">Jumlah data <?php echo $jumlah_data ?></h6> 
  ...
  <ul class="pagination justify-content-center">
    <li class="page-item">
      <a class="page-link" <?php if($page > 1){ echo "href='index.php?halaman=admin&page=$mundur'";}?> aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
      </a>
    </li>
    <li class="page-item"><div class="page-link"><?php echo $page; ?> dari <?php echo $jumlah_halaman; ?></div></li>
    <li class="page-item">
      <a class="page-link" <?php if($page < $jumlah_halaman){ echo "href='href='index.php?halaman=admin&page=$maju'";}?> aria-label="Next">
